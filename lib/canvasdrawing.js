...
CanvasDrawing.prototype.brush = {
	...
	marker: function(x, y) {
		...
		// setup brush
		this.cd.context.globalAlpha = 1;
		this.cd.context.lineCap = "round";
		
		// draw
		this.cd.context.beginPath();
		this.cd.context.moveTo(this.cd.oldX, this.cd.oldY);
		this.cd.context.lineTo(x, y);
		this.cd.context.stroke();
		
		// TODO: smooth drawing, use bezier curves, fill out lines when brush is moved fast (don't rely on lineTo)
	},
